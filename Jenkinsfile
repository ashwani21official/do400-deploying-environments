...
steps {
sh """
oc set image deployment ${DEPLOYMENT_PRODUCTION} shopping-cart-production=quay.io/${QUAY_USR}/do400-deployingenvironments:build-${BUILD_NUMBER} -n ${APP_NAMESPACE} --record
"""
}
}
}
